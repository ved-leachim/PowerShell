...
# 1.1.0 - Rewrite foreach with for loop because of memory issues (split into 2 reports)
#
# References:
...
function Get-AllPages() {
    ...
    Process {
        $AllPages = @()
        for ($i = 6321; $i -lt $AllSites.Count; $i++) {
            Write-Progress `
                -Id 0 `
                -Activity "Working on: $($AllSites[$i].url)" `
                -Status "$i of $($AllSites.Count)" `
                -PercentComplete (($i / $AllSites.Count) * 100)

            Switch ($AllSites[$i].Language) {
                1031 { 
                    $SitePages = $AllSites[$i].GetList("$($AllSites[$i].url)/Seiten") 
                    Write-Log -Level Info -Message "$($AllSites[$i].url) is a German Site."
                    break
                }
                1033 { 
                    $SitePages = $AllSites[$i].GetList("$($AllSites[$i].url)/Pages") 
                    Write-Log -Level Info -Message "$($AllSites[$i].url) is an English Site."
                    break
                }
                1036 { 
                    $SitePages = $AllSites[$i].GetList("$($AllSites[$i].url)/Pages") 
                    Write-Log -Level Info -Message "$($AllSites[$i].url) is a French Site."
                    break
                }
                default {
                    Write-Log -Level Warn -Message "$($AllSites[$i].url) has the Language Code: $($AllSites[$i].Language)! | Script did not collect any pages from this site."
                    Write-Host "$($AllSites[$i].url) has the Language Code: $($AllSites[$i].Language)! | Script did not collect any pages from this site." -ForegroundColor Yellow
                }
            }
            try {
                foreach ($Page in $SitePages.Items) {
                    $PageObject = New-Object PSObject -Property @{
                        absoluteUrl = $AllSites[$i].url + "/" + $Page.url
                ...
                Write-Log -Level Error -Message "Error during Page retrieval of $($AllSites[$i].url)"
                Write-Host "Error-Message: $($Error[0])" -ForegroundColor Red
                Write-Host "Error during Page retrieval of $($AllSites[$i].url)" -ForegroundColor Red
            }
            # Here you can split the script if you want to run it in multiple sessions
            if ($i -eq 12643) {
                return $AllPages
                break
            } 
