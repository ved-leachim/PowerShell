...
# https://learn.microsoft.com/en-us/answers/questions/1179786/exchangeonlinemanagement-authentication-broken-in
#
# Dependencies:
...
New-Variable -Name A5StudentSkuPartNumber -Value "M365EDU_A5_STUUSEBNFT" -Option Constant
New-Variable -Name A5StudentSkuPartId -Value "31d57bc7-3a05-4867-ab53-97a17835a411" -Option Constant
...
$SPOLicensePlanListName = "LIST NAME WITH THE LICENSEPLAN" # SPO List Name where License Plan is stored
$UsageLocationCode = "CH"
$TeamsInfoHook = "HTTPS://..."
$TeamsErrorHook = "HTTPS://..."
$TeamsWarningHook = "HTTPS://..."
$A5StudentRunBookUrl = "HTTPS://PORTAL.AZURE.COM/..."
#-------------------------------------------------------------#
...
Function Connect-Environments {
    ...
    try {
        ...
        Connect-ExchangeOnline -Organization $TenantName
...
Function Get-NewlySynchedUsers() {
    ...
    # Filter out users that already have an A5Student LicensePlan assigned
    $NewlySynchedUsers = $NewlySynchedUsers | Where-Object { $_.AssignedLicenses.SkuId -ne $A5StudentSkuPartId }
    ...
    if ($A5StaffSkuPartId) {
        ...
        Write-Error "Unable not load PS-Script Configuration for A5StaffSkuPartId."
        Write-BasicAdaptiveCard -ChannelHookURI $TeamsErrorHook -Message "Unable to load PS Script basic config" -OptionalMessage "Could not load 'A5StaffSkuPartId' Value." -ErrorMessage $_.Exception.Message
        throw $_.Exception
    }

    # When there are Student Accounts with Staff Licenses assigned, write a warning
...
Function Set-MailboxSettings() {
    ...
    ForEach ($User in $NewlySynchedUsers) {
        ...
        if ($HasFailed) {
            ...
            Write-FactCard -ChannelHookURI $TeamsErrorHook -Title "Failed to set Mailbox Settings" -ButtonTitle "RunBook" -ButtonUrl $A5StudentRunBookUrl -Facts $Facts
        }
        else {
